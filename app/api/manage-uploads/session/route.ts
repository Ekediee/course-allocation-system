...
export const POST = async (req: NextRequest) => {
  ...
  try {
    ...
    logger.error({ err }, 'Creating session error');
...
export const GET = async (req: NextRequest) => {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return NextResponse.json({ error: errorData.error || 'Failed to fetch session' }, { status: res.status });
    }
    ...
    logger.error({ err }, 'Fetching active session error');
    return NextResponse.json({ error: 'Server error' }, { status: 500 });
  }
};